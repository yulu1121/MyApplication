package com.example.downloadcotinue;

import android.content.Context;
import android.support.test.InstrumentationRegistry;
import android.support.test.runner.AndroidJUnit4;
import android.util.Log;

import com.example.downloadcotinue.dao.DownLoadDao;
import com.example.downloadcotinue.dao.DownLoadDaoImpl;
import com.example.downloadcotinue.enties.DownLoadObj;

import org.junit.Test;
import org.junit.runner.RunWith;

import static org.junit.Assert.*;

/**
 * Instrumentation test, which will execute on an Android device.
 *
 * @see <a href="http://d.android.com/tools/testing">Testing documentation</a>
 */
@RunWith(AndroidJUnit4.class)
public class ExampleInstrumentedTest {
    @Test
    public void useAppContext() throws Exception {
        // Context of the app under test.
        Context appContext = InstrumentationRegistry.getTargetContext();

        assertEquals("com.example.downloadcotinue", appContext.getPackageName());
    }
    @Test
    public void testDownLoad(){
        DownLoadDao dao = new DownLoadDaoImpl(InstrumentationRegistry.getTargetContext());
        DownLoadObj obj = new DownLoadObj(0,"text","text",0,0);
        dao.saveDownLoadObj(obj);
        dao.updateLength("text",999);
        dao.updateProgress("text",888);
        dao.deleteDownLoadObj("text");
    }
}
